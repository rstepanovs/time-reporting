...
import { testAccountant, testAdmin, testAdminOnly, testManager, testEmployee } from "@/test/fixtures";
...
describe("navigation", () => {
  ...
  it("shows the Billing menu with Invoices to an accountant", async () => {
    vi.mocked(fetchCurrentUser).mockResolvedValue(testAccountant);
    renderApp("/");
    await screen.findByRole("heading", { name: "Time Reporting" });

    expect(within(screen.getByRole("navigation")).getByText("Billing")).toBeTruthy();
    fireEvent.click(within(screen.getByRole("navigation")).getByText("Billing"));
    expect(await screen.findByRole("link", { name: "Invoices" })).toBeTruthy();
  });

  it("hides the Billing menu from a manager without the accountant level", async () => {
    vi.mocked(fetchCurrentUser).mockResolvedValue(testManager);
    renderApp("/");
    await screen.findByRole("heading", { name: "Time Reporting" });

    expect(within(screen.getByRole("navigation")).queryByText("Billing")).toBeNull();
  });
