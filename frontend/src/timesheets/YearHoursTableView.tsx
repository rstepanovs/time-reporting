import { Badge, Stack, Table, Text, Title, UnstyledButton } from "@mantine/core";
import { useState } from "react";

import type { MonthHours, YearHours } from "@/timesheets/api";
...
type Props = {
  data: YearHours;
  /** Overrides the default "<Year>" heading; pass "" to render no heading at all. */
  title?: string;
};

/** Presentational: renders an already-loaded year of hours. See `YearHoursTable` for the
 * data-loading wrapper most callers want. */
export function YearHoursTableView({ data, title }: Props) {
  ...
  return (
    <Stack gap="xs">
      {title !== "" && <Title order={3}>{title ?? data.year}</Title>}
      {data.months.length === 0 ? (
        ...
        <Table.ScrollContainer minWidth={720}>
          <Table withTableBorder withColumnBorders striped>
            ...
            <Table.Tfoot>
              <Table.Tr>
                <Table.Th>Total {data.year}</Table.Th>
                ...
                <YearDelta
                  totalHours={data.totals.total_hours}
                  expected={data.expected_hours_to_date}
                />
              </Table.Tr>
            </Table.Tfoot>
          </Table>
        ...
        <Table.Td>
          ...
          >
            <Text
              component="span"
              size="xs"
              style={{ visibility: hasProjects ? "visible" : "hidden" }}
            >
              {expanded ? "▾" : "▸"}
            </Text>
